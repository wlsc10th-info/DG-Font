...
from validation.validation2 import validateUN
...
parser.add_argument('--attr_size', type=int, default=400)
parser.add_argument('--wlsc_val_cnt', type=int)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Classes to use
    args.att_to_use = [i for i in range(args.attr_size)]
    ...
    content_path = Path(args.data_path) / 'id_0'
    for i in range(args.wlsc_val_cnt, 0, -1):
        ...
        if args.validation:
            validationFunc(val_loader, networks, 999, args, i, {'logger': logger})
            (content_path / f'{i:04d}.png').unlink()
